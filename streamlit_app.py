...
selected_language = st.sidebar.selectbox(
    "Choose language", 
    ["en", "es", "vi"], 
    index=["en", "es", "vi"].index(st.session_state.language),
    key="language"
)
...
def _(content, target_lang=st.session_state.language):
...
with tab2:
    ...
    st.write(_(
        "Latest model with database and history updated"
    ))
    ...
    if prompt1:
        ...
        myobj = {"prompt": prompt1, "history": st.session_state.messages1}
        stream = requests.post(url, json = myobj)
...
with tab3:
    # Show title and description.
    st.title("💬 /rag_stream_complete API")
    st.write(_(
        "Latest model with database and history updated"
    ))
